public class Sudoku {
    ...
    public static boolean place_number(int board[][],int row ,int col){
        ...
        if(board[row][col]!=0){
            return place_number(board, nextrow, nextcol);
        }
        for(int digit=1;digit<=9;digit++){
            if(isSafe(board,row,col,digit)){
                board[row][col]=digit;
                if(place_number(board, nextrow, nextcol)){
                    return true;
                } 
                board[row][col]=0;
            }
